...
import { Nav, INavLinkGroup, INavLink, SearchBox, Spinner, SpinnerSize } from "@fluentui/react";
...
const ScreenNav: React.VFC = function ScreenNav() {
  ...
  useEffect(() => {
    setLoading(true);
    const params: Parameters<typeof listApps>[0] = { page: currentPage, page_size: PAGE_SIZE };
    if (debouncedQuery) {
  ...
  const onRenderLink = useCallback(
    (
      ...
      if (link.projectId != null) {
        ...
        return (
          <div style={{ display: "flex", flexDirection: "column", gap: 2, textAlign: "left", width: "100%", margin: "6px 0" }}>
            <span style={{ fontSize: 14, fontWeight: isSelected ? 600 : 400, lineHeight: 1.2 }}>
              {link.projectId}
            </span>
      ...
      return (rendered != null && typeof rendered === "object" && "type" in rendered
        ? rendered
        : <span>{link.name}</span>) as React.ReactElement;
    },
    ...
    <div className={styles.sidebarWrap}>
      ...
      <div className={styles.sidebarNavScroll}>
        {loading ? (
          ...
          <p style={{ fontSize: 13, color: "#797775", padding: "12px 16px", margin: 0 }}>
            No projects found.
          </p>
        ) : (
          <Nav
